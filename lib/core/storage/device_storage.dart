...
class DeviceStorage {
  ...
  Future<void> saveDevices(List<DeviceModel> devices, int areaId) async {
    AppLogger.d(
      '💾 [DeviceStorage.saveDevices] 保存防区 $areaId 的 ${devices.length} 个设备',
      tag: LogTag.storage,
    );
  ...
  Future<List<DeviceModel>?> getDevices(int areaId) async {
    AppLogger.d(
      '📖 [DeviceStorage.getDevices] 读取防区 $areaId 的缓存',
      tag: LogTag.storage,
    );
    return _storage.getList(_areaKey(areaId));
  }
  ...
  List<DeviceModel>? getDevicesSync(int areaId) {
    AppLogger.d(
      '📖 [DeviceStorage.getDevicesSync] 同步读取防区 $areaId 的缓存',
      tag: LogTag.storage,
    );
